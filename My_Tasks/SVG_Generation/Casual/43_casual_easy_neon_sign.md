...
| 2   | content     | Show the word "OPEN" glowing in pink neon on the canvas on initial load                        | major  | "OPEN" is the default word displayed before any user interaction, establishing the initial state of the generator.           | C1           |
...
| 6   | state       | Select a randomly chosen word from OPEN, NEON, GLOW, VIBE, LATE when Randomize clicked        | major  | The selected word must come from the five preset options to ensure the generator cycles only through the defined words.       | C3           |
