...
| 6   | interaction | Update canvas with a randomly selected word from OPEN, NEON, GLOW, VIBE, LATE when Randomize clicked             | major  | Clicking Randomize cycles through the preset word list to produce a sign displaying a different word.         | C3           |
| 7   | visual      | Render the neon sign text with a pink glow effect                                                                  | major  | The pink glow is the core visual identity of the neon sign and makes the output immediately recognizable as neon. | C1           |
...
| 9   | state       | Retain the displayed neon sign unchanged on the canvas between Randomize clicks                                    | minor  | Sign stability between clicks gives users time to view and download the current sign before generating a new one. | C1           |
